...
2. **Dynamic least-cost path (DLCP)** 
...
2. **costSurface:** the cost surface, updated to reflect the new roads that were added.
3. **roadMethod** the road simulation method used.
4. **landings** the landings used in the simulation.
5. **g** the graph that describes the cost of paths between each cell in the cost raster. This is updated based on the new roads so that vertices connected by new roads now have a cost of 0. This can be used to avoid recomputing the graph in a simulation with multiple time steps. 
...
### 3. Dynamic Least Cost Paths (DLCP)
